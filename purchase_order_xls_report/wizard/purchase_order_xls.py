        ...
        if purchase_order:
            for rec in purchase_order:
                ...
                for rec in order_lines:
                    ...
                    sheet.write(row, 2, rec.get('date_planned'), format3)
                    ...
                    if rec.get('taxes_id'):
                        ...
                        sheet.write(row, 5, 0, format4)
                    if final_value['currency_id'].position == "before":
                        sheet.write(row, 6, final_value['currency_id'].symbol + str(rec.get('price_subtotal')), format4)
                    else:
                        sheet.write(row, 6, str(rec.get('price_subtotal')) + final_value['currency_id'].symbol, format4)
                    row += 1
                row += 2
                sheet.write(row, 5, 'UNTAXED AMOUNT', format8)
                if final_value['currency_id'].position == "before":
                    sheet.write(row, 6, final_value['currency_id'].symbol + str(final_value['amount_untaxed']), format7)
                else:
                    sheet.write(row, 6, str(final_value['amount_untaxed']) + final_value['currency_id'].symbol, format7)    
                sheet.write(row+1, 5, 'TAXES', format8)
                if final_value['currency_id'].position == "before":
                    sheet.write(row+1, 6, final_value['currency_id'].symbol + str(final_value['amount_tax']), format7)
                else:
                    sheet.write(row+1, 6, str(final_value['amount_tax']) + final_value['currency_id'].symbol, format7)
                sheet.write(row+2, 5, 'TOTAL', format8)
                if final_value['currency_id'].position == "before":
                    sheet.write(row+2, 6, final_value['currency_id'].symbol + str(final_value['amount_total']), format7)
                else:
                    sheet.write(row+2, 6, str(final_value['amount_total']) + final_value['currency_id'].symbol, format7)
        else:
